...
class IseRadiusIntegration(DnacBase):
    ...
    def accept_cisco_ise_server_certificate(self, ipAddress):
        """
        ...
        try:
            AuthServer = self.auth_server_exists(ipAddress)
            if not AuthServer:
                self.log(str("Error while retrieving the Authentication and Policy Server {0} \
                             details.".format(ipAddress)), "CRITICAL")
            ...
            if not cisco_ise_id:
                self.log(str("Error while retrieving ht Authentication and Policy Server {0} id."
                             .format(ipAddress)), "CRITICAL")
                self.msg = "Error while retrieving ht Authentication and Policy Server {0} id." \
                           .format(ipAddress)
                self.status = "failed"
                return self
    ...
    def update_auth_policy_server(self, ipAddress):
        ...
        if not self.have.get("authenticationPolicyServer").get("exists"):
            ...
            if not self.want.get("authenticationPolicyServer").get("isIseEnabled"):
            ...
            self.accept_cisco_ise_server_certificate(ipAddress)
